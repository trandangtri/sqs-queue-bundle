...
class BaseWorker
{
     ...
     */
    public function start(BaseQueue $queue, $amount = 0, $limit = 1)
    {
        $this->consumed = 0;
     ...
     * @param int $limit
     */
    private function consume(BaseQueue $queue, $amount = 0, $limit = 1)
    {
        while (true) {
     ...
     * @param int $limit
     */
    private function fetchMessage(BaseQueue $queue, $limit = 1)
    {
        ...
        while ($messages->valid()) {
            ...
            if ($result !== false) {
                $this->logger && $this->logger->info(sprintf('Successfully processed message ID: %s', $message->getId()));
                $queue->deleteMessage($message);
            } else {
                $this->logger && $this->logger->warning(sprintf('Cannot process message ID: %s, will release it back to queue', $message->getId()));
