...
class QueueListCommandTest extends KernelTestCase
{
    ...
    public function setUp()
    {
        if ($this->queueManager === null) {
            ...
            $this->queueManager->expects($this->any())->method('listQueue')->willReturnCallback(function ($arg) {
    ...
    public function testExecute()
    {
        ...
        $commandTester->execute(['--prefix' => 'queue-prefix']);
    ...
    {
        $commandTester = $this->createCommandTester(new QueueListCommand());
        $commandTester->execute(['--prefix' => 'invalid']);
        $output = $commandTester->getDisplay();
        $this->assertContains('You don\'t have any queue at this moment. Please go to AWS Console to create a new one.', $output);
    }
}