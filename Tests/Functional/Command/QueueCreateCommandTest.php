...
class QueueCreateCommandTest extends KernelTestCase
{
    ...
    public function setUp()
    {
        if ($this->queueManager === null) {
            ...
            $this->queueManager->expects($this->any())->method('createQueue')->with('my-queue-name', ['DelaySeconds' => 1, 'MaximumMessageSize' => 1, 'MessageRetentionPeriod' => 1, 'ReceiveMessageWaitTimeSeconds' => 1, 'VisibilityTimeout' => 1, 'ContentBasedDeduplication' => true])->willReturn('new-queue-url');
    ...
    public function testExecute()
    {
        ...
        $commandTester->execute(['name' => 'my-queue-name', '--delay_seconds' => 1, '--maximum_message_size' => 1, '--message_retention_period' => 1, '--receive_message_wait_time_seconds' => 1, '--visibility_timeout' => 1, '--content_based_deduplication' => true]);
